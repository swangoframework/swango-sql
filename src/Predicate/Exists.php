...
class Exists extends AbstractExpression implements PredicateInterface {
    protected $exists;
    protected $specification = 'EXISTS %s';
    protected $valueSpecSpecification = 'EXISTS (%s)';
     ...
     */
    public function __construct($exists = null, ...$valueParameter) {
        if ($exists !== null) {
     ...
     */
    public function setExists($exists, ...$valueParameter) {
        if (is_string($exists)) {
            ...
            throw new Exception\InvalidArgumentException(
                '$exists must be either an string, a Sql\Select object or a Sql\Expression object, ' . gettype($exists) .
                     ' given');
        $this->exists = $exists;
    ...
    public function getExpressionData(): array {
        ...
        $specification = vsprintf(
            $exists instanceof Select ? $this->specification : str_replace('%s', '(%s)', $this->specification),
            [
